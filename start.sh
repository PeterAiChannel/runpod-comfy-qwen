...
# CHẠY DỊCH VỤ
# =========================
# JupyterLab (background) 0.0.0.0:8888
if [ "$ENABLE_JUPYTER" = "1" ]; then
  nohup jupyter lab --ip=0.0.0.0 --port=8888 --no-browser     ${JUPYTER_TOKEN:+--ServerApp.token="$JUPYTER_TOKEN"}     > "$WORKDIR/jupyter.log" 2>&1 &
fi
